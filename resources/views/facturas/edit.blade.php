    ...
    <div class="card-body">
        <form action="{{ route('facturas.update', $Factura) }}" method="POST">
            ...
            <div class="row">
                ...
                value="{{ old('fecFac', $Factura->fecFac) }}" name="fecFac" aria-describedby="helpId"
                placeholder="Fecha Factura" />
            </div>

            <div class="row">
                <x-adminlte-input type="text" label="Nro. Factura"  fgroup-class="col-md-6"
                value="{{ old('nroFac', $Factura->nroFac) }}" name="nroFac" aria-describedby="helpId"
                placeholder="Nro Factura" />
            </div>

            <div class="row">
                <x-adminlte-input type="number" Label="Cantidad" fgroup-class="col-md-6"
                value="{{ old('cantFac', $Factura->cantFac) }}" name="cantFac" aria-describedby="helpId" placeholder="Cantidad" />
            </div>

            <div class="row">
                <x-adminlte-input type="number" step="any" label="Importe Factura." fgroup-class="col-md-6"
                value="{{ old('impFac', $Factura->impFac) }}" name="impFac" aria-describedby="helpId"
                placeholder="Importa Factura" />
            </div>

            <div class="row">
                <x-adminlte-input type="number" label="Remito ST" fgroup-class="col-md-6"
                value="{{ old('remitoST', $Factura->remST) }}" name="remST" aria-describedby="helpId"
            ...
            <div class="row">
                ...
                <option value="{{ $Factura->ocompra }}" selected="selected">{{ $Factura->ocompra }}</option>
            ...
            <div class="row">
                ...
                <option value="{{ $Factura->remK }}" selected="selected">{{ $Factura->remK }}</option>
